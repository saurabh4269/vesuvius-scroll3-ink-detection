#!/bin/bash
#SBATCH --job-name=ensemble_s3
#SBATCH --partition=a40
#SBATCH --qos=a40
#SBATCH --nodes=1
#SBATCH --ntasks-per-node=8
#SBATCH --gres=gpu:1
#SBATCH --time=00:30:00
#SBATCH --mem=96G
#SBATCH --output=logs/infer_ensemble_%J.out
#SBATCH --error=logs/infer_ensemble_%J.err

export PATH=$PATH:/opt/slurm/bin
set -eo pipefail

source ~/miniconda3/etc/profile.d/conda.sh
conda activate scroll

export WANDB_MODE=offline
export TRANSFORMERS_OFFLINE=1
export HF_DATASETS_OFFLINE=1
export TRAINING_PRECISION=32
export CUDA_VISIBLE_DEVICES=0
CUDNN_LIB="$HOME/miniconda3/envs/scroll/lib/python3.10/site-packages/nvidia/cudnn/lib"
export LD_LIBRARY_PATH="${CUDNN_LIB}:${LD_LIBRARY_PATH:-}"

PROJ="$HOME/scroll_prize/vesuvius_first_title_prize"
CFG="${PROJ}/configs/ft_esrf.py"
MINIUNETR_CKPT="${PROJ}/checkpoints/ft_esrf_manual_20260531_003040/best_epoch_015_val_loss_0.6041.pt"
TIMESFORMER_CKPT="${HOME}/scroll_prize/villa/ink-detection/timesformer_wild15_20230702185753_0_fr_i3depoch=12.ckpt"
SEGMENT_DIR="$HOME/scroll_prize/data/scroll3/fragments/20240618142020"
OUTPUT_DIR="${PROJ}/results/scroll3_ensemble_$(date +%Y%m%d_%H%M%S)"

mkdir -p ~/scroll_prize/logs
cd "${PROJ}"

echo "=== Ensemble Inference: MiniUNETR + TimeSformer ==="
echo "MiniUNETR checkpoint: ${MINIUNETR_CKPT}"
echo "TimeSformer checkpoint: ${TIMESFORMER_CKPT}"
echo "Segment: ${SEGMENT_DIR}"
echo "Output: ${OUTPUT_DIR}"
echo ""

python -u scripts/infer_s3_ensemble.py \
    "${CFG}" \
    "${MINIUNETR_CKPT}" \
    "${TIMESFORMER_CKPT}" \
    --segment-dir "${SEGMENT_DIR}" \
    --output-dir "${OUTPUT_DIR}" \
    --ensemble-weight 0.6

echo ""
echo "=== Ensemble inference complete ==="
echo "Results saved to: ${OUTPUT_DIR}"
echo ""
echo "Evaluating results..."
python -u scripts/evaluate_ensemble.py "${OUTPUT_DIR}"
